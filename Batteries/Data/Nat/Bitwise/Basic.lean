...
@[simp, grind =] theorem isOdd_add_two : isOdd (n + 2) = isOdd n := rfl

@[grind =] theorem isOdd_succ : isOdd (n + 1) = !(isOdd n) := by fun_induction isOdd <;> grind

@[grind =_]
theorem isOdd_val : isOdd n = n.testBit 0 := by fun_induction isOdd <;> grind
@[csimp] theorem isOdd_eq_isOddImpl : isOdd = isOddImpl := funext (fun _ => isOdd_val)

/-! ### isEven -/

@[simp, grind =] theorem isEven_zero : isEven 0 = true := rfl
@[simp, grind =] theorem isEven_one : isEven 1 = false := rfl
@[simp, grind =] theorem isEven_add_two : isEven (n + 2) = isEven n := rfl

@[grind =] theorem isEven_succ : isEven (n + 1) = !(isEven n) := by fun_induction isEven <;> grind

theorem isEven_val : isEven n = !(n.testBit 0) := by fun_induction isEven <;> grind
@[csimp] theorem isEven_eq_isEvenImpl : isEven = isEvenImpl := funext (fun _ => isEven_val)

/-! ### isOdd, isEven -/

theorem not_isOdd : (!(isOdd n)) = isEven n := by fun_induction isOdd <;> grind

@[simp, grind =]
theorem isEven_eq : isEven n = !(isOdd n) := not_isOdd.symm

theorem not_isEven : (!(isEven n)) = isOdd n := by simp
...
@[grind =] theorem div2_succ : div2 (n + 1) = n.div2 + n.isOdd.toNat := by induction n <;> grind
...
@[simp, grind =] theorem bit_succ : bit b (n + 1) = n.bit b + 2 := rfl
...
theorem bit_val : n.bit b = 2 * n + b.toNat := congrFun (congrFun bit_eq_bitImpl b) n

theorem bit_false : bit false n = 2 * n := bit_val
theorem bit_true : bit true n = 2 * n + 1 := bit_val
...
@[simp] theorem bit_eq_zero_iff : n.bit b = 0 ↔ n = 0 ∧ b = false := bit_inj n 0 b false
theorem bit_ne_zero_iff : n.bit b ≠ 0 ↔ n ≠ 0 ∨ b = true := by grind [bit_eq_zero_iff]

@[grind =>]
theorem ne_zero_of_bit_ne_zero (hn : n.bit b = 0) : n = 0 := by grind [bit_eq_zero_iff]
...
@[grind =]
theorem bit_le_bit_of_eq : bit b n ≤ bit b m ↔ n ≤ m := by grind [Nat.le_iff_lt_or_eq]

@[grind =]
theorem zero_lt_bit_iff : 0 < bit b k ↔ 0 < k ∨ b = true := by grind

@[simp]
theorem zero_lt_bit_false : 0 < bit false k ↔ 0 < k := by grind

@[simp]
theorem zero_lt_bit_true : 0 < bit true k := by grind

/-! ### div2, isOdd, bit -/

@[simp, grind =] theorem isOdd_bit {b : Bool} (n : Nat) : (n.bit b).isOdd = b := by
  fun_induction bit <;> grind [cases Bool]

theorem isEven_bit {b : Bool} (n : Nat) : (n.bit b).isEven = !b := by simp
...
theorem testBit_bit_zero {n : Nat} : (n.bit b).testBit 0 = b := by grind
theorem testBit_bit_add_one {n : Nat} : (n.bit b).testBit (m + 1) = n.testBit m := by grind

@[grind =]
theorem testBit_bit {n : Nat} : (n.bit b).testBit m = if m = 0 then b else n.testBit (m - 1) := by
  grind [cases Nat]
...
theorem binaryInduction_bit_zero :
    binaryInduction (motive := motive) zero one bit (Nat.bit b 0) =
    if h : b = true then h ▸ one else b.of_not_eq_true h ▸ zero := by grind

theorem binaryInduction_bit_add_two {n : Nat} :
    binaryInduction (motive := motive) zero one bit (n.bit b + 2) =
    bit (n.bit b) (n.div2_bit.symm ▸ binaryInduction zero one bit (n + 1)) := by grind
...
theorem binaryElim_succ :
    ...
    bit (!n.isOdd) (binaryElim zero one bit (n.div2 + n.isOdd.toNat)) := by
  cases n with | zero => grind | succ n => cases h : n.isOdd <;> grind
...
@[grind =]
theorem size_succ : size (n + 1) = if n = 0 then 1 else size (n.div2 + n.isOdd.toNat) + 1 :=
  binaryElim_succ
...
theorem size_bit_of_ne_zero (hn : n ≠ 0) : size (n.bit b) = size n + 1 := by grind
@[simp] theorem size_bit_of_neZero [NeZero n] : size (n.bit b) = size n + 1 :=
...
theorem popcount_succ : popcount (n + 1) = if n = 0 then 1 else
    popcount (n.div2 + n.isOdd.toNat) + (!n.isOdd).toNat := binaryElim_succ
...
theorem popcount_le_size : popcount n ≤ size n := by
  induction n using binaryInduction <;> grind
...
@[simp, grind =] theorem bitsList_add_two : bitsList (n + 2) = n.isOdd ::
    bitsList (n.div2 + 1) := by simp [bitsList]

@[grind =]
theorem bitsList_succ : bitsList (n + 1) = if n = 0 then [true] else
  (!n.isOdd) :: bitsList (n.div2 + n.isOdd.toNat) := binaryElim_succ
...
theorem bitsList_bit_of_ne_zero (hn : n ≠ 0) : bitsList (n.bit b) = b :: bitsList n := by grind
@[simp] theorem bitsList_bit_of_neZero [NeZero n] : bitsList (n.bit b) = b :: bitsList n :=
...
@[simp] theorem ofBitsList_eq_zero_iff : ofBitsList bs = 0 ↔ true ∉ bs := by
  induction bs <;> grind

@[grind .]
theorem ofBitsList_ne_zero_iff : ofBitsList bs ≠ 0 ↔ true ∈ bs := by simp

@[grind .]
theorem true_mem_of_ne_zero_ofBitsList (h : ofBitsList bs ≠ 0) : true ∈ bs := by simpa using h

@[grind .]
theorem true_mem_of_neZero_ofBitsList [NeZero (ofBitsList bs)] : true ∈ bs :=
  true_mem_of_ne_zero_ofBitsList (NeZero.ne _)

theorem ne_zero_ofBitsList_of_true_mem (h : true ∈ bs) : ofBitsList bs ≠ 0 := by simpa using h
